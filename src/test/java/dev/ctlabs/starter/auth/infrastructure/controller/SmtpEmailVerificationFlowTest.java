...
class SmtpEmailVerificationFlowTest {
    ...
    void registerShouldGenerateCodeWhenProviderIsSmtp() throws Exception {
        var request = new RegisterRequest("Smtp", "User", "smtp@test.com", null, "Password123!");
        ...
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() -> {
            ...
            assertThat(receivedMessages[receivedMessages.length - 1].getSubject())
    ...
    void loginShouldFailWhenEmailNotVerified() throws Exception {
        var registerRequest = new RegisterRequest("Smtp", "Login", "smtp_login@test.com", null, "Password123!");
        ...
        var loginRequest = new LoginRequest("smtp_login@test.com", "Password123!");
    ...
    void verifyEmailShouldWorkWithValidCode() throws Exception {
        var registerRequest = new RegisterRequest("Verify", "User", "verify@test.com", null, "Password123!");
    ...
    void forgotPasswordShouldSendEmail() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() -> {
            ...
            assertThat(receivedMessages)
                    .extracting(msg -> msg.getAllRecipients()[0].toString())
                    .contains("forgot_email@test.com");
        });
    }
    ...
    void resetPasswordShouldWorkWithValidCode() throws Exception {
        ...
        var loginRequest = new LoginRequest("reset_pass@test.com", "NewPassword123!");
        mockMvc.perform(post("/api/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(loginRequest)))
                .andExpect(status().isOk());

        var oldLoginRequest = new LoginRequest("reset_pass@test.com", "Password123!");
        mockMvc.perform(post("/api/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
